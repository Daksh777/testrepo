...
const PublicTransportFirstChart = ({ data }: ChildData) => {
  const cityChartWednesday =
    data?.["8Cc"]?.map((value: any, ind: number) => ({
      hour: ind + 5,
      bus: parseInt(value),
    })) || [];
  const cityChartSunday =
    data?.["8Cd"]?.map((value: any, ind: number) => ({
      hour: ind + 5,
      bus: parseInt(value),
    })) || [];
...
export default PublicTransportFirstChart;
